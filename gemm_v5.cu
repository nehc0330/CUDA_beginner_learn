...
__global__ void gemm_v5(int M, int K, int N, float *d_A, float *d_B, float *d_C)
{
    ...
    for (int inner_k = 0; inner_k < K_per_BLOCK; inner_k++)
    {
        ...
        // 利用寄存器
        for (int kk = 0; kk < K_per_BLOCK; kk++)
        {
            float a_val[4] = {
                A_block[ty * NUM_per_THREAD][kk],
                A_block[ty * NUM_per_THREAD + 1][kk],
                A_block[ty * NUM_per_THREAD + 2][kk],
                A_block[ty * NUM_per_THREAD + 3][kk]};
            float b_val[4] = FETCH_FLOAT4(B_block[kk][tx * NUM_per_THREAD]);
            for (int i = 0; i < NUM_per_THREAD; i++)
            {
